...
def descompress_file(file, destination):
    file = tarfile.open(file)
    file.extractall(destination)
    file.close()

def copy_file(source_folder, destination_folder):
    cmd = 'cp '+source_folder+' '+destination_folder
    os.system(cmd) 
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--meka_classpath', type=str, required=False, default='/home/dellvale/scikit_ml_learn_data/meka/meka-release-1.9.8/lib/')
    parser.add_argument('--meka_classpath', type=str, required=False, default='./meu-meka-1.9.8/lib/')
    parser.add_argument('--java_command', type=str, required=False, default='/home/dellvale/miniconda3/envs/GeneralEnv/bin/java')
    ...
    parser.add_argument('--scratch', type=str, required=False, default= '/media/dellvale/Dados/Doutorado/Experimentos/Experimento4/scratch/emanuel')
    ...
    parser.add_argument('--mlc_limit_time', type=int, required=False, default=45)
    parser.add_argument('--len_population', type=int, required=False, default=9)
    parser.add_argument('--number_generation', type=int, required=False, default=2)
    ...
    # pasta datasets
    create_dir(args.scratch, 'datasets')
    path_dataset = args.scratch+'/datasets'

    copy_file(args.project+'/datasets/'+args.name_dataset+'.tar.gz', path_dataset)
    descompress_file(path_dataset+'/'+args.name_dataset+'.tar.gz', path_dataset)

    # pasta datasets temporários
    create_dir(path_dataset, 'tempdatasets')

    # pasta resultados
    create_dir(args.scratch, 'results')
    create_dir(args.scratch+'/results', args.name_dataset)
    path_results = args.scratch+'/results/'+args.name_dataset
    ...
    output_data += f'Number MLC exception: {problem.n_mlc_exceptions}\n'
